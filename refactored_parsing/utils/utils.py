...
def get_argparse_type_for_container(
    ...
    Arguments:
        container_type: A container type (ideally a typing.Type such as List, Tuple, along with \
            an item annotation: List[str], Tuple[int, int], etc.)

    Returns:
        the type that should be used in argparse 'type' argument option.
...
def is_dataclass_or_optional_dataclass_type(t: type) -> bool:
    ...
    return dataclasses.is_dataclass(t) or (
        is_optional(t) and dataclasses.is_dataclass(get_args(t)[0])
    )
