        ...
        for part in keys[:-1]:
            ...
            if cur_field.metadata.get("subgroups") and len(sub_fields) == 0:
                ...
                if cur_field.default is MISSING and cur_field.default_factory is MISSING:
                    raise ValueError(
                        f"The choice of the subgroups '{part}' is not provided.")
                else:
                    sub_cls = cur_field.metadata["subgroups"][cur_field.metadata["subgroup_default"]]
                    sub_fields[part] = ({f.name: ({}, f)
                                        for f in fields(sub_cls)}, cur_field)
        else:
            key = keys[-1]
            if key in sub_fields and sub_fields[key][1].metadata.get('subgroups'):
                ...
                if isinstance(value, str):
                    ...
                    sub_fields[key] = ({f.name: ({}, f)
                                       for f in fields(sub_cls)}, my_field)
                    sub_dictionary = sub_dictionary.setdefault(
                        f"__subgroups__@{key}", value)
                else:
...
def replace(
    ...
    new_dataclass_dict = dict_union(
        dataclass_dict, unflatten_new_values, recurse=True)
    return from_dict(
