...
def _update_schema_with_descriptions(
    ...
    for classname, definition in definitions.items():
        ...
        # description.
        docstring = inspect_getdoc(definition_dc_type)
        if docstring is not None and not _has_default_dataclass_docstring(definition_dc_type):
            definition.setdefault("description", docstring)

        if "properties" in definition:
            assert isinstance(definition["properties"], dict)
            dc_fields = {field.name: field for field in dataclasses.fields(definition_dc_type)}
                ...
                if property_name not in dc_fields:
                    logger.warning(
                        RuntimeWarning(
                            ...
                            f"property {property_name} which isn't a field of the dataclass {definition_dc_type}"
                        )
                    )
                    continue
                field_docstring = get_attribute_docstring(definition_dc_type, property_name)
