...
from simple_parsing.utils import (DataclassT, contains_dataclass_type_arg,
                                  is_dataclass_instance, is_dataclass_type,
                                  is_optional, is_union, unflatten_keyword)

logger = logging.getLogger(__name__)


@overload
def replace_selections(obj: DataclassT, changes_dict: dict[str, Any], selections: dict[str, Key | DataclassT] | None = None) -> DataclassT:
    ...


@overload
def replace_selections(obj: DataclassT, selections: dict[str, Key | DataclassT] | None = None, **changes) -> DataclassT:
    ...


def replace_selected_dataclass(obj: DataclassT, selections: dict[str, Key | DataclassT] | None = None):
    """
    ...
    for field in dataclasses.fields(obj):
        ...
        if contains_dataclass_type_arg(t) and is_union(t):
            ...
            if child_selections:
                new_value = replace_selected_dataclass(
                    field_value, child_selections)
            else:
...
def replace_selections(obj: DataclassT, changes_dict: dict[str, Any] | None = None, selections: dict[str, Key | DataclassT] | None = None, **changes) -> DataclassT:
