...
class FieldWrapper(Wrapper[dataclasses.Field]):
    ...
    def get_arg_options(self) -> Dict[str, Any]:
        ...
        if utils.is_choice(self.field) or utils.is_literal(self.type):
            if utils.is_choice(self.field):
                ...
                if utils.is_list(self.type):
                    ...
                    _optional_args(_arg_options, choice_types)
            else:
                _literal_args(_arg_options, self.type)
            ...
            if utils.is_literal(type_arguments[0]):
                _arg_options["required"] = False
                _arg_options["nargs"] = argparse.OPTIONAL

                _literal_args(_arg_options, type_arguments[0])
            else:
                _optional_args(_arg_options, type_arguments)
            ...
            type_arguments = utils.get_args(self.type)[0]
            if utils.is_literal(type_arguments):
                _literal_args(_arg_options, type_arguments)
            ...
            type_arguments = utils.get_args(self.type)
            if type_arguments and utils.is_literal(type_arguments[0]):
                _literal_args(_arg_options, type_arguments[0])
...
def _optional_args(_arg_options, type_arguments):
    _arg_options["required"] = False
...
def _literal_args(_arg_options, t):
    type_arguments = utils.get_type_arguments(t)

    if len(set(type(arg) for arg in type_arguments)) != 1:
        raise ValueError("Only Literals of the same type are supported")

    literal_type = type(type_arguments[0])

    if literal_type not in [str, int, bool, bytes] and utils.is_enum(literal_type):
        raise ValueError("Only strings, integers, booleans, bytes and enums are supported in Literals")

    _arg_options["type"] = literal_type
    _arg_options["choices"] = type_arguments