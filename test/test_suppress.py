...
def test_suppress_conflict_with_unsuppressed_dataclass(simple_attribute, silent):
    ...
    assert parser.parse_args(
        shlex.split(f"--some_class2.a {passed_value}")
    ) == argparse.Namespace(some_class2=SomeClass(expected_value))
...
def test_suppress_with_regular_suppressed_args(simple_attribute, silent):
    ...
    assert parsed_args == argparse.Namespace(
        some_class={"a": expected_value}, c=expected_value
    )
    assert unparsed_args == []
...
def test_suppress_default_nested(simple_attribute, silent):
    ...
    parsed_args, unparsed_args = parser.parse_known_args(
        shlex.split(f"--c {passed_value} --b 0")
    )
    assert parsed_args == argparse.Namespace(
        some_class={"a": {"c": expected_value}, "b": 0}
    )
    assert unparsed_args == []
