...
def test_suppress_with_unsuppressed_dataclass(simple_attribute, silent):
    ...
    assert parsed_args == argparse.Namespace(some_class1={'a': expected_value, 'b': 0},
                                             some_class2=SomeClass2(expected_value))
    assert unparsed_args == []

    parsed_args, unparsed_args = parser.parse_known_args(shlex.split(f'--a {passed_value} --c {passed_value}'))
    assert parsed_args == argparse.Namespace(some_class1={'a': expected_value}, some_class2=SomeClass2(expected_value))
    assert unparsed_args == []
...
def test_suppress_conflict_with_unsuppressed_dataclass(simple_attribute, silent):
    ...
    parsed_args, unparsed_args = parser.parse_known_args(
        shlex.split(f'--some_class1.a {passed_value} --some_class1.b 0 --some_class2.a {passed_value}')
    )
    assert parsed_args == argparse.Namespace(some_class1={'a': expected_value, 'b': 0},
                                             some_class2=SomeClass(expected_value))
    assert unparsed_args == []

    parsed_args, unparsed_args = parser.parse_known_args(shlex.split(f'--a {passed_value} --some_class2.a {passed_value}'))
