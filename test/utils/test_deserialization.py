...
def test_backward_compatibility_success_cases(config: object):
    new_config = from_dict(
        config.__class__, to_dict(config, add_selected_subgroups=False), drop_extra_fields=True, parse_selection=False
    )
...
def test_with_selection_info(config: object):
    new_config = from_dict(
        config.__class__, to_dict(config, add_selected_subgroups=True), drop_extra_fields=True, parse_selection=True
    )
    assert config == new_config
